...
bool process_registerApp_ack(int32_t csocket )
{
    ...
    bool success = true;
    uint32_t retBytes, k, actualLength;
...
bool process_registerData_ack(int32_t csocket )
{
    ...
    bool success = true;
    uint32_t retBytes, k, errCnt, actualLength, calcLength;
...
void process_publish( int32_t csocket , struct sockaddr_in addr_in , int32_t LogMPs[] , int32_t time_secMP[] , int32_t time_nsecMP[] )
{
    ...
    // currerntly hardcoded ... need to build these MPs based on what's received from SUBSCRIBE.  TODO
    // MP 1
...
//bool process_sysInitComplete( int32_t csocket , struct sockaddr_in addr_in )
//{
//    enum sysinit_params
//    {
//        CMD_ID                  = 2,
...
bool process_subscribe( int32_t csocket )
{
    ...
    uint8_t host_os;
    uint16_t seq_num;
    uint32_t src_proc_id, src_app_name, cnt_retBytes;
    ...
    else
    {
        ...
        //  printf("SIZE OF MPinfo: %lu\n", sizeof(MPinfo));
            ...
            subscribeMP = (MPinfo *)realloc( subscribeMP , sizeof(MPinfo) * 7);
        ...
        //  printf("SIZE OF FOR subscribeMP: %lu\n", sizeof(subscribeMP));
        //  printf("SIZE OF MPinfo: %lu\n", sizeof(MPinfo));
...
bool process_subscribe_ack( int32_t csocket , struct sockaddr_in addr_in )
{
    ...
    *ptr = getpid();
    ...
    for ( i = 0 ; i < num_mps ; i++ ) 
    {
        if (true == check_validMP( subscribeMP[i].mp , subscribeMP[i].period ) )
        {
    ...
    *ptr = MSG_SIZE;
    ...
    toSendUDP_size = sizeof(addr_in);
    sendBytes = sendto(csocket, sendData, cntBytes, 0, (struct sockaddr *)&addr_in, toSendUDP_size);
    //sendBytes = sendto(csocket, sendData, cntBytes, 0, (struct sockaddr *)&addr_in, sizeof(addr_in));
...
bool check_validMP( int32_t myMP , int32_t myMPperiod )
{
    ...
    {
        if ( ( SIMMsubscriptionMP[i] == myMP ) && ( ( SIMMsubscriptionPeriod[i] % myMPperiod ) == 0 ) )
        {
            // valid MP and valid MP period
            break;
        }
        else if ( ( SIMMsubscriptionMP[i] == myMP ) && ( ( SIMMsubscriptionPeriod[i] % myMPperiod ) != 0 ) ) 
        {
