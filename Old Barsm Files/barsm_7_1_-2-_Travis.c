...
int check_modules(void)
{
    ...
    for (i = 0; i < p; i++)
    {
        child_status = check_process(child_pid[i]);
        
        // Check if the process failed and was previously failed:
        if (1 == child_status && 1 == status[i])
        {
            // increment total number of failures
            fails++;
            printf("ERROR: Child process with PID: %d has ended.\n", child_pid[i]);

            // one more child process failed
            printf("Failed children: %d\n", fails);
        ...
        if (fails == p)
        {
            // all child processes failed
            printf("ERROR: All child processes failed!\n");
            return(-1);
        }
    }
    return(fails);
...
// 
// IN:  pid
//      - Receives a process ID to check the existence of.
// 
// OUT: int
//      - Returns a 1 if process does not exist, and a 0 if it does.
//      ** I've got some printf usage to help debug for now ... will be sent to syslog() as well
// 
//  ... this will go in a .h ????
//===========================================================================================================================================
