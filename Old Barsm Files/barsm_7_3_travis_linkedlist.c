...
int main()
{
    ...
    while(-1 != stopped_pid)
    {
        ...
        // restart any processes that have gone zombie
        if(stopped_pid > 0)
        {
...
int check_modules(void)
{
    ...
    if (-1 == waitreturn) 
    {
        printf("\nERROR: waitpid returned -1 error: (%d:%s) \n", errno, strerror(errno));
        sleep(1);
    }
    else if (0 < waitreturn)
    {
        printf("\nERROR: Child process with PID %d was a zombie: process eliminated.\n", waitreturn);
        return(waitreturn);
    }
    else
    {
...
void restart_process(int s_pid, child_pid_list **first_node_ptr, child_pid_list **nth_node_ptr)
{
    ...
    size_t length = strlen(nth_node->dir);
    char *temp_dir = (char*) malloc(length + 1);
    length = strlen(nth_node->item_name);
    char *temp_name = (char*) malloc(length + 1);
